...
export default class VirtualKeyboard {
  ...
  constructor(element, lang, targetElement) {
    ...
    let tlang = lang;
    if (lang === 'load') {
      const l = this.langStorage.read();
      if (l) tlang = l;
      else {
        tlang = 'en';
        this.langStorage.write(tlang);
      }
    }
    this.state = createState(
      ...
      },
      {
        ...
        language: [
          (value) => this.#changeLangOnAllKeys(value),
          (value) => this.langStorage.write(value),
        ],
  ...
  #lockKey(key) {
    ...
    if (key.config.special && key.config.special.state) this.state[key.config.special.state] = true;
  }
  ...
  #onMouseDown(event, key) {
    ...
    if (key.config.special && key.config.special.clicklock === true) this.#lockKey(key);
  }

  #onMouseUp(event, key) {
    if (key.state.pressed) this.#generalKeyUp(key);
  }

  #onKeyDown(event) {
    const { code } = event;
    const key = this.keys.find((k) => k.config.id === code);
    if (!key) return;
